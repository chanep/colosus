...
class Evaluator:
    ...
    def evalueate(self, games: int, iterations: int, position_ini: Position, weights_filename, weights_filename2):
        ...
        for game_num in range(games):
            ...
            print("game: {}, tow_win_rate: {}, mates_move_mean: {}".format(game_num + 1, win_rate_two, mates_move_mean))
    ...
    def get_state(self, game_num: int, move_num: int, position: Position):
        if self.is_two(game_num, move_num):
            return State2(position.clone(), None, None, self.colosus2)
        else:
            return State(position.clone(), None, None, self.colosus)
